...
class NoMaD(nn.Module):
    ...
    def forward(self, func_name, **kwargs):
        if func_name == "vision_encoder":
            if (hasattr(self.vision_encoder, "include_action_in_state_decoder")
                    and self.vision_encoder.include_action_in_state_decoder):
                output = self.vision_encoder(
                    kwargs["obs_img"],
                    kwargs["goal_img"],
                    input_goal_mask=kwargs["input_goal_mask"],
                    previous_action_angle=kwargs["previous_action_angle"],
                )
            else:
                output = self.vision_encoder(
                    kwargs["obs_img"],
